...
const { TOTAL_HEADER, CALCULATE_TOTALS_HEADER, CALCULATE_ONLY_TOTALS_HEADER, RETURN_ONLY_PARAMS_HEADER } = require('./data-helpers/headers');
...
module.exports = class ApiListData extends API {
	...
	async process() {
		...
		try {
			...
			const formattedFilters = await this.formatFilters(getParams.filters, getParams.order);

			if(this.isValidObject(formattedFilters))
				getParams.filters = formattedFilters;

			if(this.shouldReturnOnlyParams())
				return this.setBody({ params: getParams, noResults: !!this.noResults });

			if(this.noResults)
				return this.setBody({ result: [], total: 0 });
			...
			const { result, total } = await this.fetchData(getParams, totalsLimit);
