...
module.exports = class ApiListData extends API {
	...
	async fetchData(getParams, totalsLimit) {
		...
		if(this.shouldReturnBody) {
			...
			const limit = totalsLimit || getParams.limit;

			// If result is less than limit in the first page, set total to result length to avoid a second request to fetch totals
			if(getParams.page === 1 && data.result.length < limit) {
