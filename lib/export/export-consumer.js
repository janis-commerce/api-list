...
// Time budget for a single Lambda invocation (12 min by default). Below the 900s Lambda timeout defined in
// the hooks, leaving margin to send the part message and let batch orchestrate the resume.
const DEFAULT_GENERATION_BUDGET_MS = 720 * 1000;
...
module.exports = class ExportConsumer extends IterativeSQSConsumer {
	...
	/**
	 * Read per-invocation (not at module load) so the budget can be tuned hot via env var
	 * without a code deploy — e.g. lowered to force the cut with small datasets when testing.
	 */
	get generationBudgetMs() {
		return Number(process.env.EXPORT_GENERATION_BUDGET_MS) || DEFAULT_GENERATION_BUDGET_MS;
	}
	...
	isBudgetExceeded(processingStart, lastPartClosedAt) {
		...
		return elapsed + (lastPartDuration * PART_DURATION_SAFETY_FACTOR) > this.generationBudgetMs;
	}
